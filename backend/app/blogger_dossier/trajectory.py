...
DIST_BINS = 8             # 表现分布(图B)的对数分箱数


def _pct(sorted_vals: list[float], q: float) -> float:
    """线性插值分位数(q∈[0,1]),输入需已排序。空→0,单值→自身。"""
    if not sorted_vals:
        return 0.0
...
def _distribution(likes: list[int]) -> dict[str, Any]:
    """表现分布(图B):分位数 + 对数分箱直方图。赞跨数量级,线性分箱没意义,用 log10。"""
    s = sorted(likes)
    n = len(s)
    if n == 0:
        return {"count": 0}
    ...
    if hi_v > lo_v:
        lo_l, hi_l = math.log10(max(lo_v, 1)), math.log10(max(hi_v, 1))
        step = (hi_l - lo_l) / DIST_BINS or 1.0
        edges = [10 ** (lo_l + i * step) for i in range(DIST_BINS + 1)]
        counts = [0] * DIST_BINS
        for v in s:
            idx = int((math.log10(max(v, 1)) - lo_l) / step) if step else 0
            counts[min(max(idx, 0), DIST_BINS - 1)] += 1
        bins = [{"lo": round(edges[b]), "hi": round(edges[b + 1]), "count": counts[b]} for b in range(DIST_BINS)]
    return {
        "count": n, "min": lo_v, "p25": round(_pct(s, 0.25)), "median": round(_pct(s, 0.5)),
        "p75": round(_pct(s, 0.75)), "p90": round(_pct(s, 0.9)), "max": hi_v, "log_bins": bins,
    }
...
def build_trajectory(posts: list[BloggerPost]) -> dict[str, Any]:
    ...
    distribution = _distribution([pt["like"] for pt in points]) if points else {"count": 0}
    if len(points) < MIN_POINTS:
        return _degraded(points, f"有发布时间的笔记仅 {len(points)} 篇，暂不划分阶段", distribution)
    ...
    if len(buckets) < MIN_BUCKETS:
        out = _degraded(points, f"按{'季' if quarterly else '月'}分桶仅 {len(buckets)} 段，暂只给整体趋势", distribution)
...
def _degraded(points: list[dict[str, Any]], summary: str, distribution: dict[str, Any] | None = None) -> dict[str, Any]:
    return {"points": points, "buckets": [], "bursts": [], "phases": [], "level_ups": [],
            "recent_trend": "", "granularity": "", "low_frequency": False, "summary": summary,
            "distribution": distribution or {"count": 0}}
...
def _bucketize(dated: list[BloggerPost]) -> tuple[list[dict[str, Any]], bool]:
    ...
    for key in order:
        ...
        sorted_likes = sorted(note_likes)
        buckets.append({
            ...
            "p25": round(_pct(sorted_likes, 0.25), 1),
            "p75": round(_pct(sorted_likes, 0.75), 1),
            "note_count": len(ps),
            "_likes": note_likes,
            "_posts": ps,
        })
