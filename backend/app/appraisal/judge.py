...
def judge_vertical(titles: list[str], settings: Settings, *, timeout: int | None = None) -> JudgedDim:
    """垂直度:近 N 篇标题是否聚焦一个赛道。"""
    joined = "\n".join(f"- {t}" for t in titles if t)[:3000]
    if not joined:
        return JudgedDim("vertical", "垂直度", 50, "无标题可判,给中性分")
    try:
        data = create_json_response(settings, _VERTICAL_PROMPT.format(titles=joined), timeout=timeout)
    except Exception as exc:  # noqa: BLE001
        logger.warning("垂直度判断失败,中性兜底:%s", exc)
        return JudgedDim("vertical", "垂直度", 50, "模型判断超时或出错,暂给中性分,可稍后重试")
...
def _soft_fallback(msg: str) -> dict[str, JudgedDim]:
    """三项软实力都给中性分 + 同一句说明(区分:没填意图 / 没笔记 / 真失败)。"""
    return {
        "fit": JudgedDim("fit", "对路", 50, msg),
        "learnable": JudgedDim("learnable", "可学", 50, msg),
        "distillable": JudgedDim("distillable", "可蒸馏", 50, msg),
    }


def judge_soft(intent: str, notes_brief: str, settings: Settings, *, timeout: int | None = None) -> dict[str, JudgedDim]:
    """软实力(诊断别人用):对路(三面法)/ 可学 / 可蒸馏。失败 → 三项中性兜底。"""
    if not (intent or "").strip():
        return _soft_fallback("未填写「想学什么 / 对标意图」,无法判断你能不能学 TA;在上方填写后重新诊断即可")
    if not (notes_brief or "").strip():
        return _soft_fallback("没采到可用的笔记内容,无法判断;请先到博主资产里多采集一些笔记")
    ...
    try:
        ...
        return _soft_fallback("模型判断超时或出错,暂给中性分,可稍后重试")
    if not isinstance(data, dict):
        return _soft_fallback("模型返回格式异常,暂给中性分,可稍后重试")
...
def judge_note_relevance(
    intent: str, titles: list[str], settings: Settings, *, timeout: int | None = None
) -> list[dict[str, Any]]:
    ...
    try:
        data = create_json_response(settings, _RELEVANCE_PROMPT.format(intent=intent.strip(), titles=listed), timeout=timeout)
    except Exception as exc:  # noqa: BLE001
