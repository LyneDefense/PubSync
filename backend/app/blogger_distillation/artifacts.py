...
def render_report_html(blogger: BloggerProfile, stats: dict[str, Any], distillation: dict[str, Any], usage: TikHubUsage) -> str:
    sections = [
        ...
        ("视频口播/字幕结构", distillation.get("video_script_structures")),
    ...
    body = [
        f"<h1>{html.escape(blogger.display_name)} 小红书内容蒸馏报告</h1>",
        f"<p>样本 {stats.get('sample_count', 0)} 条，采样评论 {stats.get('comment_total', 0)} 条，TikHub 请求 {usage.request_count} 次，估算费用 ${usage.estimated_cost_usd:.4f}（区间 ${usage.cost_min_usd:.4f} - ${usage.cost_max_usd:.4f}）。</p>",
    ]
...
def build_skill_markdown(blogger: BloggerProfile, stats: dict[str, Any], distillation: dict[str, Any]) -> str:
    ...
    return f"""---
name: {name}
description: 基于小红书博主 {blogger.display_name} 公开内容蒸馏出的创作方法论。不要冒充原博主，不要复制原文。
---

# {blogger.display_name} 小红书内容方法论 Skill

## 使用说明

你不是原博主本人，不要冒充原博主。你学习的是公开内容里的选题、结构、表达节奏、封面文案和读者洞察。输出时必须服务于用户自己的账号定位，不能复用原文、原图、私密经历或身份。

适用平台：小红书图文、视频口播脚本，也可以迁移到公众号短内容选题。

样本规模：{stats.get("sample_count", 0)} 条笔记，{stats.get("comment_total", 0)} 条采样评论。视频样本 {transcript_info.get("video_count", 0)} 条，已解析字幕/口播 {transcript_info.get("transcript_count", 0)} 条。
...
{markdown_list(distillation.get("video_script_structures"))}
...
def render_hot_posts(posts: list[dict[str, Any]]) -> str:
    ...
    for index, item in enumerate(posts[:10], 1):
        ...
        items.append(
            f"{index}. {item.get('title', '')} | 点赞 {item.get('like_count', 0)} / 收藏 {item.get('favorite_count', 0)} / {comment_label}"
        )
    return "\n".join(items)
