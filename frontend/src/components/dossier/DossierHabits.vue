...
// 模态偏好(视频vs图文均赞)已在数据面板环图展示,这里不重复。
...
<template>
  <section v-if="rows.length" class="dossier-block">
    <header class="dossier-block__head">
      <h3>运营习惯</h3>
      <span class="dossier-block__hint">事实主体 + 解读 · 依据详情级 {{ habits.coverage?.detail ?? 0 }} 篇</span>
    </header>
    <div class="dh-list">
      <div v-for="r in rows" :key="r.label" class="dh-row">
        <div class="dh-fact">
          <span class="dh-label">{{ r.label }}</span>
          <span class="dh-value">{{ r.value }}</span>
        </div>
        <p v-if="r.interp" class="dh-interp"><em>解读</em>{{ r.interp }}</p>
      </div>
    </div>
  </section>
</template>
...
.dh-row { padding: 12px 0; border-top: 1px solid var(--color-rule); }
.dh-row:first-child { border-top: 0; }
.dh-fact { display: flex; justify-content: space-between; align-items: baseline; gap: 12px; }
.dh-label { font-size: 12.5px; color: var(--color-ink-3); flex-shrink: 0; }
.dh-value { font-size: 13.5px; color: var(--color-ink); text-align: right; }
.dh-interp { display: flex; gap: 6px; align-items: baseline; margin: 5px 0 0; font-size: 12.5px; color: var(--color-ink-2); line-height: 1.5; }
.dh-interp em {
  flex-shrink: 0;
  font-style: normal;
  font-size: 11px;
