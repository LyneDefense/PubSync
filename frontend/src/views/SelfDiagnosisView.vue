...
const STEP_LABELS = ['选择我的账号', '明确意图', '诊断报告']
...
const selectedAccount = computed(() => myAccounts.value.find((a) => a.id === selfAppraiseForm.blogger_id) || null)
const selectedName = computed(() => selectedAccount.value?.display_name || '')
...
async function checkIntent() {
  ...
  startElapsedTimer()
  try {
    const ctx = await fetchIntentContext(selfAppraiseForm.blogger_id)
    noteCount.value = ctx.note_count
    stageStep.value = 1
    await fetchSelfIntentSuggestions()
    quizIndex.value = 0
...
// 引导失败时「跳过,直接诊断」:视为目标已明确,进 quiz(0 题)直接可开始诊断。
function skipGuide() {
  markSelfIntentSkipped()
  analyzeError.value = ''
  quizIndex.value = 0
...
// —— 答题打卡(自诊断:目标 / 痛点 / 阶段) ——
const curQuestion = computed<AppraisalIntentQuestion | null>(() => selfIntentQuestions.value[quizIndex.value] || null)
const answeredQuestions = computed(() => selfIntentQuestions.value.slice(0, quizIndex.value))
const pendingQuestions = computed(() => selfIntentQuestions.value.slice(quizIndex.value + 1))
const isLastQuestion = computed(() => quizIndex.value >= selfIntentQuestions.value.length - 1)
const curAnswered = computed(() => {
  if (!curQuestion.value) return false
  const sel = selfIntentSelections[quizIndex.value] || []
  const other = (selfIntentOthers[quizIndex.value] || '').trim()
  return sel.length > 0 || other.length > 0
})

function isPicked(qi: number, opt: string): boolean {
  return (selfIntentSelections[qi] || []).includes(opt)
}
function pickOption(q: AppraisalIntentQuestion, opt: string) {
  ...
  if (q.multi === false) {
    selfIntentSelections[qi] = idx >= 0 ? [] : [opt] // 单选:点已选取消,点别的替换
  } else if (idx >= 0) {
    arr.splice(idx, 1)
  } else {
    arr.push(opt)
...
function answerOf(qi: number): string[] {
  const picked = [...(selfIntentSelections[qi] || [])]
  const other = (selfIntentOthers[qi] || '').trim()
...
function nextQuiz() {
  if (quizIndex.value < selfIntentQuestions.value.length - 1) quizIndex.value += 1
}
...
function pickAccount(id: number) {
  selfAppraiseForm.blogger_id = selfAppraiseForm.blogger_id === id ? 0 : id
...
async function exportImage() {
  ...
  try {
    ...
    a.href = dataUrl
    a.download = `${reportAccountName.value}-账号诊断报告.png`
    a.click()
    showMessage('已导出报告图片')
  } catch (err) {
...
<template>
  <section v-if="isSocialPlatform && currentSocialTab === 'self-diagnosis'" class="analysis">
    <header class="page-head">
      <h1>诊断我的{{ currentSocialPlatformName }}账号</h1>
      <p>给自己的号做体检 —— 硬实力(体量 / 互动 / 爆款 / 活跃 / 垂直)× 合规 × 目标契合。诊断前会自动确保 ≥ 20 篇笔记样本。</p>
    </header>
    ...
    <template v-if="step === 1">
      <section class="card">
        <div class="card-head">
          ...
          <span class="head-hint">{{ selectedAccount ? `已选择「${selectedName}」` : '请选择一个要诊断的账号' }}</span>
        </div>
        <div v-if="myAccounts.length" class="blogger-grid">
          <button
            v-for="acc in myAccounts"
            :key="acc.id"
            type="button"
            class="blogger-card"
            :class="{ sel: selfAppraiseForm.blogger_id === acc.id }"
            @click="pickAccount(acc.id)"
          >
            <span class="avatar" :style="avatarStyle(acc.id)">{{ (acc.display_name || '?').slice(0, 1) }}</span>
            <span class="b-body">
              <span class="b-name">{{ acc.display_name }}</span>
              <span class="b-meta">
                <span class="niche" :class="{ muted: !acc.niche }">{{ acc.niche || '未设置领域' }}</span>
                <span class="samples">样本 {{ acc.sample_count }}</span>
              </span>
            </span>
            <span class="radio" :class="{ on: selfAppraiseForm.blogger_id === acc.id }">✓</span>
          </button>
        </div>
      ...
      <!-- 找差距(我 vs 对标):下一轮上线,诚实占位 -->
      <section class="card gap-card">
        <div class="card-head">
          <h2>找差距(我 vs 对标)</h2>
          <span class="head-hint">即将上线</span>
        </div>
        <p class="empty-region pad">下一轮上线:选一个对标博主 + 我的账号,量化差距、给整改清单。</p>
      </section>
    </template>

    <!-- ===== Step 2 · 明确诊断意图(意图录入 → 内嵌进度 → 答题打卡) ===== -->
    <section v-else-if="step === 2" class="card">
      <div class="card-head col">
        <p class="kicker">第 2 步 / 共 3 步</p>
        <h2>明确你的诊断意图</h2>
        <p class="sub">
          正在诊断「<b>{{ selectedName }}</b>」。诊断自己的号要先知道你的目标和痛点。先说一句,我们会看你在做什么、再用几道选择题帮你把目标理清。
        </p>
      </div>
      ...
      <template v-if="intentPhase === 'intro'">
        <div class="card-body form">
          <label class="field">
            <span>你的目标 / 现在的痛点 <em>(可不填)</em></span>
            <textarea
      ...
      <div v-else-if="intentPhase === 'analyzing'" class="card-body analyze">
        <div class="progress-card" :class="{ failed: !!analyzeError }" role="status" aria-live="polite">
          <div class="pc-head">
            ...
            <span class="pc-title">{{ analyzeError ? '分析没能完成' : '正在看你最近在做什么…' }}</span>
          ...
          <template v-if="!analyzeError">
            ...
            <p class="pc-hint">通常需要 5–10 秒;你的笔记越多,问得越准。</p>
          </template>
          <template v-else>
      ...
      <div v-else class="card-body quiz">
        <!-- 顶部进度 -->
        <div v-if="selfIntentQuestions.length" class="quiz-progress">
          <span class="qp-label">明确意图 · 已答 {{ quizIndex }} / {{ selfIntentQuestions.length }}</span>
          <div class="qp-track">
            <i v-for="i in selfIntentQuestions.length" :key="i" :class="segClass(i - 1)"></i>
          </div>
        </div>
        ...
        <div v-if="curQuestion" class="current">
          ...
          <input
            ...
            v-model="selfIntentOthers[quizIndex]"
            class="cur-other"
            type="text"
            placeholder="✎ 其他,自己填…"
          />
    ...
    <template v-else>
      ...
      <div v-if="report" ref="reportEl" class="report-capture">
        <div class="capture-head">
          <div class="ch-left">
            <strong>{{ reportAccountName }}</strong>
            <span>账号诊断 · 基于 {{ report.sample_count }} 篇笔记</span>
          </div>
