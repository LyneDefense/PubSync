...
// 候选 | 已选 两列工作台,泛搜索与找相似共用。候选→采用进已选 / 不要剔除;已选→保存进对标库。
        ...
        <div class="dw-body">
          <article v-for="c in ws.candidates" :key="c.external_id" class="dw-card">
            ...
            <div class="dw-card-main">
              <div class="dw-card-top"><strong>{{ c.display_name }}</strong><span class="dw-score">匹配度 {{ Math.round(c.score) }}</span></div>
              <div class="dw-data">{{ fmtFans(c) }} · <span :class="c.is_personal ? 'ok' : 'muted'">{{ c.is_personal ? '个人号' : '机构号' }}</span><span v-if="c.existing_blogger_id" class="dw-tag">已在库</span></div>
              <div class="dw-reason">{{ c.reason }}</div>
        ...
        <div class="dw-body">
          <article v-for="b in ws.selected" :key="b.external_id" class="dw-card">
            <div class="dw-card-main">
              ...
              <div class="dw-data">{{ fmtFans(b) }} · <span :class="b.is_personal ? 'ok' : 'muted'">{{ b.is_personal ? '个人号' : '机构号' }}</span></div>
              <div class="dw-card-acts">
...
.dw-cols { display: grid; grid-template-columns: 1.3fr 1fr; gap: 12px; }
...
.dw-body { display: flex; flex-direction: column; gap: 8px; max-height: 520px; overflow-y: auto; }
...
.dw-card img { width: 36px; height: 36px; border-radius: 50%; flex: none; object-fit: cover; }
...
.dw-score { font-size: 0.72rem; color: var(--color-accent, #2563eb); white-space: nowrap; }
...
.dw-reason { font-size: 0.76rem; color: var(--color-ink-3, #9aa0a6); margin-top: 3px; }
